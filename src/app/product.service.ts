import { Injectable } from '@angular/core';
import { BehaviorSubject, Observable } from 'rxjs';
import { Character } from './Model/character.model';

@Injectable({
  providedIn: 'root',
})
export class ProductService {
  private characters: Character[] = [
    {
      id: 0,
      houseColor: 'Gryffondor',
      name: 'Harry Potter',
      image: 'assets/images/harrypotter.jpg',
      isFavorite: false,
      createdAt: new Date(1980, 6, 31),
      price: 99.99,
      wand: 'Bois de houx, plume de phénix, 27,5 cm',
      patronus: 'Cerf',
      year: 1,
      role: "L'Élu",
      description:
        "Le célèbre garçon qui a survécu, marqué par une cicatrice en forme d'éclair. Orphelin, il découvre qu'il est un sorcier le jour de ses 11 ans. Courageux et loyal, il est destiné à affronter Lord Voldemort.",
    },
    {
      id: 1,
      houseColor: 'Gryffondor',
      name: 'Ronald Weasley',
      image: 'assets/images/Ron_Weasley.jpg',
      isFavorite: false,
      createdAt: new Date(1980, 3, 1),
      price: 79.99,
      wand: 'Bois de saule, crin de licorne, 35 cm',
      patronus: 'Jack Russell Terrier',
      year: 1,
      role: 'Préfet de Gryffondor',
      description:
        'Sixième enfant de la famille Weasley, Ron est le meilleur ami de Harry. Loyal et courageux malgré ses insécurités, il se distingue par son talent aux échecs version sorcier et son humour.',
    },
    {
      id: 2,
      houseColor: 'Gryffondor',
      name: 'Hermione Granger',
      image: 'assets/images/Hermione_Granger.jpg',
      isFavorite: false,
      createdAt: new Date(1979, 8, 19),
      price: 89.99,
      wand: 'Bois de vigne, ventricule de dragon, 27,3 cm',
      patronus: 'Loutre',
      year: 1,
      role: 'Préfète de Gryffondor',
      description:
        "Née de parents moldus, Hermione est la sorcière la plus brillante de sa génération. Passionnée par les études et dotée d'une intelligence remarquable, elle est une amie fidèle de Harry et Ron.",
    },
    {
      id: 3,
      houseColor: 'Gryffondor',
      name: 'Neville Londubat',
      image: 'assets/images/Neville_Londubas.jpg',
      isFavorite: false,
      createdAt: new Date(1980, 7, 30),
      price: 72.99,
      wand: 'Bois de cerisier, crin de licorne, 33 cm',
      patronus: 'Non spécifié',
      year: 1,
      role: 'Leader de la résistance à Poudlard',
      description:
        "D'abord maladroit et manquant de confiance, Neville devient un leader courageux. Son talent en botanique et sa bravoure face à l'adversité en font un héros inattendu.",
    },
    {
      id: 4,
      houseColor: 'Gryffondor',
      name: 'Albus Dumbledore',
      image: 'assets/images/Albus_Dumbledore.jpg',
      isFavorite: false,
      createdAt: new Date(1881, 7, 30),
      price: 99.99,
      wand: 'Baguette de Sureau',
      patronus: 'Phénix',
      year: 0,
      role: 'Directeur de Poudlard',
      description:
        "Le plus grand sorcier de tous les temps, directeur de Poudlard et mentor de Harry. Sa sagesse et ses secrets guident l'histoire jusqu'à son terme.",
    },
    {
      id: 5,
      houseColor: 'Serpentard',
      name: 'Severus Rogue',
      image: 'assets/images/Severus_Snape.jpg',
      isFavorite: false,
      createdAt: new Date(1960, 1, 9),
      price: 95.99,
      wand: 'Matériaux inconnus',
      patronus: 'Biche',
      year: 0,
      role: 'Maître des Potions',
      description:
        'Professeur de potions puis de défense contre les forces du mal, Rogue est un personnage complexe dont la loyauté et les motivations restent longtemps mystérieuses.',
    },
    {
      id: 6,
      houseColor: 'Serpentard',
      name: 'Drago Malefoy',
      image: 'assets/images/Draco_Malfoy.jpg',
      isFavorite: false,
      createdAt: new Date(1980, 5, 5),
      price: 85.99,
      wand: "Bois d'aubépine, crin de licorne, 25 cm",
      patronus: 'Inconnu',
      year: 1,
      role: 'Préfet de Serpentard',
      description:
        "Héritier de la noble famille Malefoy, Drago est l'rival de Harry à Poudlard. Arrogant et préjugé, il évolue au fil des années face aux choix difficiles qui s'imposent à lui.",
    },
    {
      id: 7,
      houseColor: 'Serdaigle',
      name: 'Luna Lovegood',
      image: 'assets/images/Luna_Lovegood.jpg',
      isFavorite: false,
      createdAt: new Date(1981, 2, 13),
      price: 75.99,
      wand: 'Matériaux inconnus',
      patronus: 'Lièvre',
      year: 4,
      role: "Membre de l'A.D.",
      description:
        "Surnommée 'Loufoca', Luna est une élève unique en son genre. Son esprit ouvert et sa sagesse cachée en font une amie précieuse, malgré ses croyances excentriques.",
    },
    {
      id: 8,
      houseColor: 'Gryffondor',
      name: 'Ginny Weasley',
      image: 'assets/images/Ginny_Weasley.jpg',
      isFavorite: false,
      createdAt: new Date(1981, 7, 11),
      price: 76.99,
      wand: "Bois d'if, composant inconnu",
      patronus: 'Cheval',
      year: 2,
      role: 'Poursuiveuse de Quidditch',
      description:
        'Benjamine des Weasley, Ginny est une sorcière talentueuse et déterminée. Excellente joueuse de Quidditch, elle se révèle être une combattante redoutable.',
    },
    {
      id: 9,
      houseColor: 'Gryffondor',
      name: 'Fred Weasley',
      image: 'assets/images/Fred_Weasley.jpg',
      isFavorite: false,
      createdAt: new Date(1978, 3, 1),
      price: 82.99,
      wand: 'Matériaux inconnus',
      patronus: 'Non spécifié',
      year: 5,
      role: 'Co-fondateur de Weasley & Weasley',
      description:
        'Jumeau de George, Fred est un farceur né. Son esprit créatif et son sens des affaires ont donné naissance aux Farces pour sorciers facétieux.',
    },
    {
      id: 10,
      houseColor: 'Gryffondor',
      name: 'George Weasley',
      image: 'assets/images/George_Weasley.jpg',
      isFavorite: false,
      createdAt: new Date(1978, 3, 1),
      price: 82.99,
      wand: 'Matériaux inconnus',
      patronus: 'Non spécifié',
      year: 5,
      role: 'Co-fondateur de Weasley & Weasley',
      description:
        'Jumeau de Fred, George partage son esprit espiègle et entrepreneurial. Ensemble, ils créent joie et rires même dans les moments les plus sombres.',
    },
    {
      id: 11,
      houseColor: 'Gryffondor',
      name: 'Minerva McGonagall',
      image: 'assets/images/Minerva_McGonagall.jpg',
      isFavorite: false,
      createdAt: new Date(1935, 9, 4),
      price: 15,
    },
    {
      id: 12,
      houseColor: 'Gryffondor',
      name: 'Rubeus Hagrid',
      image: 'assets/images/Hagrid.jpg',
      isFavorite: false,
      createdAt: new Date(1928, 11, 6),
      price: 86.99,
      wand: 'Chêne, 40 cm (brisée)',
      patronus: 'Non spécifié',
      year: 0,
      role: 'Garde-chasse de Poudlard',
      description:
        "Demi-géant au grand cœur, Hagrid est le premier contact de Harry avec le monde des sorciers. Sa passion pour les créatures magiques n'a d'égale que sa loyauté envers Dumbledore.",
    },
    {
      id: 13,
      houseColor: 'Gryffondor',
      name: 'Sirius Black',
      image: 'assets/images/Sirius_Black.jpg',
      isFavorite: false,
      createdAt: new Date(1960, 10, 11),
      price: 92.99,
      wand: 'Matériaux inconnus',
      patronus: 'Chien',
      year: 0,
      role: 'Parrain de Harry',
      description:
        'Meilleur ami des parents de Harry, injustement emprisonné à Azkaban. Son évasion et sa réhabilitation marquent profondément la vie de son filleul.',
    },
    {
      id: 14,
      houseColor: 'Gryffondor',
      name: 'Remus Lupin',
      image: 'assets/images/Remus_Lupin.jpg',
      isFavorite: false,
      createdAt: new Date(1960, 2, 10),
      price: 88.99,
      wand: 'Bois de cyprès, crin de licorne, 25,6 cm',
      patronus: 'Loup',
      year: 0,
      role: 'Professeur de DCFM',
      description:
        'Loup-garou et ancien ami des parents de Harry, Lupin est le meilleur professeur de défense contre les forces du mal que Poudlard ait connu.',
    },
    {
      id: 15,
      houseColor: 'Poufsouffle',
      name: 'Cedric Diggory',
      image:
        'data:image/jpeg;base64,',
      isFavorite: false,
      createdAt: new Date(1977, 9, 1),
      price: 20,
    },
    {
      id: 16,
      houseColor: 'Poufsouffle',
      name: 'Nymphadora Tonks',
      image: 'assets/images/NymphadoraTonks.jpg',
      isFavorite: false,
      createdAt: new Date(1973, 1, 1),
      price: 5,
    },
    {
      id: 17,
      houseColor: 'Serdaigle',
      name: 'Cho Chang',
      image: 'assets/images/Cho-Chang.jpg',
      isFavorite: false,
      createdAt: new Date(1979, 8, 1),
      price: 69.99,
      wand: 'Matériaux inconnus',
      patronus: 'Cygne',
      year: 4,
      role: 'Attrapeuse de Serdaigle',
      description:
        "Attrapeuse talentueuse de l'équipe de Quidditch de Serdaigle, Cho est une élève populaire qui traverse des moments difficiles après la mort de Cedric Diggory.",
    },
    {
      id: 18,
      houseColor: 'Serpentard',
      name: 'Bellatrix Lestrange',
      image: 'assets/images/BellatrixLestrange.jpg',
      isFavorite: false,
      createdAt: new Date(1951, 1, 1),
      price: 91.99,
      wand: 'Bois de noyer, cœur de dragon, 32,5 cm',
      patronus: 'Inconnu',
      year: 0,
      role: 'Mangemort',
      description:
        "Fanatique dévouée à Voldemort, Bellatrix est l'une des Mangemorts les plus dangereuses et cruelles. Sa folie n'a d'égale que sa puissance magique.",
    },
  ];
  private charactersSubject = new BehaviorSubject<Character[]>(this.characters);
  private favoritesCountSubject = new BehaviorSubject<number>(0);

  private cartItems: { product: Character; quantity: number }[] = [];
  private cartItemCountSubject = new BehaviorSubject<number>(0);

  constructor() {
    this.loadFavoritesFromLocalStorage();

    // Garder le code existant pour le panier
    if (this.localStorageAvailable) {
      const savedCart = window.localStorage.getItem('cart');
      if (savedCart) {
        this.cartItems = JSON.parse(savedCart);
        this.updateCartItemCount();
      }
    }
  }

  private get localStorageAvailable(): boolean {
    return typeof window !== 'undefined' && !!window.localStorage;
  }

  private saveCartToLocalStorage() {
    if (this.localStorageAvailable) {
      window.localStorage.setItem('cart', JSON.stringify(this.cartItems));
    }
  }

  // Méthode pour ajouter un produit au panier
  addToCart(product: Character, quantity: number): void {
    const existingItem = this.cartItems.find(
      (item) => item.product.id === product.id
    );
    if (existingItem) {
      existingItem.quantity += quantity;
    } else {
      this.cartItems.push({ product, quantity });
    }
    this.saveCartToLocalStorage();
    this.updateCartItemCount();
  }

  // Méthode pour obtenir les éléments du panier
  getCartItems(): { product: Character; quantity: number }[] {
    return this.cartItems;
  }

  // Méthode pour calculer le total du panier
  calculateTotal(): number {
    return this.cartItems.reduce(
      (total, item) => total + item.product.price * item.quantity,
      0
    );
  }

  // Méthode pour vider le panier
  clearCart(): void {
    this.cartItems = [];
    this.saveCartToLocalStorage();
    this.updateCartItemCount();
  }

  // Obtenir tous les personnages
  getProducts(): Character[] {
    return this.characters;
  }

  // Observable pour les personnages
  getProductsObservable(): Observable<Character[]> {
    return this.charactersSubject.asObservable();
  }

  // Obtenir un personnage par ID
  getProduct(id: number): Character | undefined {
    return this.characters.find((char) => char.id === id);
  }

  // Basculer l'état favori d'un personnage
  toggleFavorite(character: Character): void {
    character.isFavorite = !character.isFavorite;
    this.updateFavoritesCount();
    this.saveFavoritesToLocalStorage();
    this.charactersSubject.next(this.characters);
  }

  // Obtenir le nombre de favoris
  getFavoritesCount(): Observable<number> {
    return this.favoritesCountSubject.asObservable();
  }

  // Obtenir uniquement les personnages favoris
  getFavorites(): Character[] {
    return this.characters.filter((char) => char.isFavorite);
  }

  // Mettre à jour le compteur de favoris
  private updateFavoritesCount(): void {
    const count = this.characters.filter((char) => char.isFavorite).length;
    this.favoritesCountSubject.next(count);
  }

  // Rechercher des personnages
  searchCharacters(term: string): Character[] {
    if (!term.trim()) {
      return this.characters;
    }

    term = term.toLowerCase();
    return this.characters.filter(
      (char) =>
        char.name.toLowerCase().includes(term) ||
        char.houseColor.toLowerCase().includes(term)
    );
  }

  // Trier les personnages
  sortCharacters(
    characters: Character[],
    sortType: 'name' | 'date',
    ascending: boolean
  ): Character[] {
    return [...characters].sort((a, b) => {
      if (sortType === 'name') {
        return ascending
          ? a.name.localeCompare(b.name)
          : b.name.localeCompare(a.name);
      } else {
        const comparison = a.createdAt.getTime() - b.createdAt.getTime();
        return ascending ? comparison : -comparison;
      }
    });
  }

  updateCartItemQuantity(product: Character, quantity: number): void {
    const existingItem = this.cartItems.find(
      (item) => item.product.id === product.id
    );
    if (existingItem) {
      existingItem.quantity = quantity;
      this.saveCartToLocalStorage();
      this.updateCartItemCount();
    }
  }

  removeFromCart(product: Character): void {
    this.cartItems = this.cartItems.filter(
      (item) => item.product.id !== product.id
    );
    this.saveCartToLocalStorage();
    this.updateCartItemCount();
  }

  // Ajouter cette méthode pour obtenir le nombre d'articles
  getCartItemCount(): Observable<number> {
    return this.cartItemCountSubject.asObservable();
  }

  // Mettre à jour cette méthode pour mettre à jour le compteur
  private updateCartItemCount(): void {
    const count = this.cartItems.reduce(
      (total, item) => total + item.quantity,
      0
    );
    this.cartItemCountSubject.next(count);
  }

  private saveFavoritesToLocalStorage(): void {
    if (this.localStorageAvailable) {
      const favorites = this.characters
        .filter((char) => char.isFavorite)
        .map((char) => char.id);
      window.localStorage.setItem('favorites', JSON.stringify(favorites));
    }
  }

  private loadFavoritesFromLocalStorage(): void {
    if (this.localStorageAvailable) {
      const savedFavorites = window.localStorage.getItem('favorites');
      if (savedFavorites) {
        const favoriteIds = JSON.parse(savedFavorites) as number[];
        this.characters.forEach((char) => {
          char.isFavorite = favoriteIds.includes(char.id);
        });
        this.updateFavoritesCount();
      }
    }
  }
}
